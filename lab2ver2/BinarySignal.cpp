...
{	// как вариант перевести в строку из 0 и 1, вставить в нужное место и использовать конструктор от строки
	...
	Signal remainingPartSignal = signals_[offset.index];	// остаток от сигнала
	signals_[offset.index].duration = position - offset.length;
	signals_[offset.index+1+binarySignalToInsert.size_].duration = signals_[offset.index].duration - position - offset.length;
	for (int i = offset.index + 2; i < offset.index + 2 + binarySignalToInsert.size_; i++) {	// сдвиг части массива
		signals_[i + binarySignalToInsert.size_] = signals_[offset.index];
	}
	...
	size_ = offset.index + 2 + binarySignalToInsert.size_;
